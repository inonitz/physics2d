...
#define TEXTURE_IMAGE_UNIT_SHADER_ACCESS_RO   ( (u16)GL_READ_ONLY  )
#define TEXTURE_IMAGE_UNIT_SHADER_ACCESS_WO   ( (u16)GL_WRITE_ONLY )
#define TEXTURE_IMAGE_UNIT_SHADER_ACCESS_RW   ( (u16)GL_READ_WRITE )
...
struct TextureDescriptor {
    std::vector< std::pair<TextureParameter, u32> > param;
    TextureInternalFormat   fmt{DEFAULT32};
    TextureInternalDatatype dtype{DEFAULT32};
...
struct TextureImageUnitDescriptor
{
    using ImageUnit      = u32;
    using mipMapLevel    = u16;
    using ShaderAccess   = u16;
    using InternalFormat = u32;

    ImageUnit      imgUnit;
    mipMapLevel    level;
    ShaderAccess   access;
    InternalFormat formatDataType ; /* GL_RGBA + GL_FLOAT = GL_RGBA32F */
};
...
public:
    ...
    TextureImageUnitDescriptor boundImageUnit;
    ...
    __force_inline void bindToImageUnit(TextureImageUnitDescriptor const& desc) {
        boundImageUnit = desc;
        glBindImageTexture(
            boundImageUnit.imgUnit, 
            id, 
            (i32)boundImageUnit.level,
            GL_FALSE,
            0, /* since layers only exist in textures with arrays, and since this class is TEXTURE_2D only, layer should stay 0 */
            (u32)boundImageUnit.access,
            boundImageUnit.formatDataType
        );
        return;
    }


    __force_inline void unbindFromImageUnit()
    {
        glBindImageTexture(
            boundImageUnit.imgUnit,
            0,
            ...
            GL_FALSE,
            0,
            TEXTURE_IMAGE_UNIT_SHADER_ACCESS_RO,
            GL_RGB
        );
        boundImageUnit = {
            boundImageUnit.imgUnit,
