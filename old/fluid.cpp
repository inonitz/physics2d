...
math::vec2f Fluid2D::divergence(VectorField* u, u32 i, u32 j) const 
{
    u32 xIdx = j + i * N; /* Linearize the indices for each velocity array */
    u32 yIdx = i + j * N; /* Linearize the indices for each velocity array */
    xIdx = xIdx + (xIdx / N);
    yIdx = yIdx + (yIdx / N);
    return { 
        recpdx * (u->x[xIdx + 1] - u->x[xIdx]),
        recpdx * (u->y[yIdx + 1] - u->y[yIdx]),
    };
}
