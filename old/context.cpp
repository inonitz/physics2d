...
void GLObjects::initialize()
{
	...
	buffers[0].create(
		...
		VERTEX_DATATYPE_F32, 
		4,
		(void* const)vertices.begin().base()
	);
	buffers[1].create(
...
void GLObjects::bind()
{
	const u32 textureUnit 		  = 1;
	// const u32 imageUnit   		  = 0;
	// const u16 mipmapLevel 		  = 0;
	// const u16 imgUnitShaderAccess = TEXTURE_IMAGE_UNIT_SHADER_ACCESS_RO;
	// const u32 fmtdtype 			  = GL_RGBA32F;

	bindDefaultShader();
	quad.bindToTextureUnit(textureUnit);
	// quad.bindToImageUnit({
	// 	imageUnit,
	// 	mipmapLevel,
	// 	imgUnitShaderAccess,
	// 	fmtdtype
	// });
	quad.setActiveTexture(GL_TEXTURE0 + textureUnit);
	vao.bind();
	visual.setUniform1i("texData", textureUnit);
}
...
void Context::render()
{
	glDrawElements(
		GL_TRIANGLES, 
		gl.buffers[1].meta.count, 
		gl.buffers[1].meta.glType(), 
		nullptr
	);
	return;
}
