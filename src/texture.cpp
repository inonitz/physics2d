...
void Texture2D::recreateImage(void* cpuBuffer)
{
    ...
    glTexImage2D(
        GL_TEXTURE_2D,
        0,
        TextureDescriptor::InternalFormatToNumOfComponents(desc.fmt),
        dimensions[0],
        dimensions[1],
        0,
        desc.fmt,
...
void Texture2D::fromFile(
    ...
    manvec<f32> texBufferFloats{}; /* use the buffer immediately and wait for out-of-scope de-alloc */


    textureBuffer = stbi_load(path.begin(), &dimx, &dimy, &channels, desiredChannels);
    texBufferFloats.increaseTo((size_t)dimx * dimy * desiredChannels * sizeof(f32) );
    TextureConvertRGBAUI32ToRGBA32F(
        textureBuffer,
        texBufferFloats.begin(),
        dimx,
        dimy
    );
...
void TextureConvertRGBAUI32ToRGBA32F(u8* inData, float* outData, i32 dimx, i32 dimy)
{
    ...
    //             };
    //             memcpy(&inData[currPixel], tmpf.begin(), sizeof(f32) * 4);
