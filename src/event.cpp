...
template<typename T> std::array<T, 2> getCurrentFrameCursorPos()  { 
	return { 
		__scast(T, MouseListener.currentFramePos[0]), 
		__scast(T, MouseListener.currentFramePos[1])
	}; 
}
template<typename T> std::array<T, 2> getPreviousFrameCursorPos() { 
	return { 
		__scast(T, MouseListener.previousFramePos[0]), 
		__scast(T, MouseListener.previousFramePos[1])
	};    
}
template<typename T> std::array<T, 2> getCursorDelta() 			  { 
	return {
		__scast(T, (MouseListener.currentFramePos[0]  - MouseListener.previousFramePos[0]) ),
		__scast(T, (MouseListener.previousFramePos[1] - MouseListener.currentFramePos[1] ) ) /* Y axis is flipped on GLFW (X_axis = right, Y_axis = down) */
	};
}
